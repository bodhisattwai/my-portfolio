        ...
        <div key={pub.id} onClick={() => openPubinNewTab(pub.url)}>
          ...
          <div className="publication-details">
            <p
              ...
              style={{ color: theme.secondaryText, fontSize: '14px', fontStyle: 'italic' }}
            >
              {pub.journal || 'Academic Journal'}
            </p>
            <p
              className="publication-creation-date subTitle"
              style={{ color: theme.secondaryText, fontSize: '12px' }}
            >
              {pub.publishedDate || pub.createdAt ? new Date(pub.publishedDate || pub.createdAt).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'long',
                day: 'numeric'
              }) : 'Date not available'}
            ...
            {pub.url && (
              ...
              >
                <span role="img" aria-label="Document">📄</span> Read Full Paper
              </a>
            )}
          </div>
