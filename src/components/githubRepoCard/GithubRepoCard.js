        ...
        <div key={repo.id} onClick={() => openRepoinNewTab(repo.url)}>
          ...
          <div className="repo-details">
            <p className="repo-creation-date" style={{ color: theme.secondaryText }}>
              {repo.createdAt ? new Date(repo.createdAt).toLocaleDateString('en-US', { 
                year: 'numeric', 
                month: 'short',
                day: 'numeric'
              }) : 'Date not available'}
            ...
            <div className="repo-languages">
              {repo.languages && repo.languages.slice(0, 3).map((lang, idx) => (
                <span key={idx} className="repo-language-tag" style={{ color: theme.text, backgroundColor: theme.highlight }}>
                  {lang.name}
                </span>
              ))}
            </div>
